...
/**
 * Taken from https://github.com/Cawfree/Dynamic-Time-Warping
 *
...
public final class DTW {
    ...
    public static DTW.Result computeMfccDtw(final List<float[]> pSample, final List<float[]> pTemplate) {
        ...
        for(i = 0; i < lN; i++) {
            ...
            for(j = 0; j < lM; j++) {
                ...
                lL[i][j] = getDistanceBetweenMfccs(pSample.get(i), pTemplate.get(j));
            }
        }
        ...
        return new DTW.Result(reverse(lWarpingPath, lK), ((lG[lN - 1][lM - 1]) / lK));
    }
    ...
    protected static double getDistanceBetween(double p1, double p2) {
        // Calculate the square error.
        return (p1 - p2) * (p1 - p2);
    }

    protected static double getDistanceBetweenMfccs(float[] mfcc1, float[] mfcc2) {
        double distance = 0;
    ...
    protected static final int getMinimumIndex(final double[] pArray) {
        // Declare iteration variables.
        int    lIndex = 0;
